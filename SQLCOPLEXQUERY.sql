...
SELECT DIRECTOR FROM MOVIEDETAILS 
...
SELECT (YEAR(GETDATE()) - RELEASEYEAR) AS YEARSDIFF FROM MOVIEDETAILS;
...
--7.5.
SELECT LANGUAGE, MIN(DURATIONMIN) AS MINA, MAX(DURATIONMIN) AS MAXA,AVG(DURATIONMIN) AS AVGA
...
SELECT M1.DIRECTOR, M1.TITLE AS MOVIE1, M1.GENRE AS GENRE1,
       M2.TITLE AS MOVIE2, M2.GENRE AS GENRE2
