...
SELECT CITY,COUNT(DEPARTMENT) FROM EMPLOYEE
GROUP BY CITY
...
SELECT DEPARTMENT FROM EMPLOYEE
GROUP BY DEPARTMENT
HAVING SUM(CASE WHEN GENDER = 'MALE' THEN 1 END)>SUM(CASE WHEN GENDER = 'FEMALE' THEN 1 END)

----Part – D:

-- 1. Display each department and the total hours worked.
SELECT Department, SUM(Hours_Worked) AS Total_Hours FROM Project_Assignments
GROUP BY Department;
...
SELECT Department, AVG(Hours_Worked) AS Avg_Hours FROM Project_Assignments
GROUP BY Department
ORDER BY Avg_Hours DESC;

-- 4. Display departments having more than 3 employees.
SELECT Department, COUNT(*) AS Employee_Count FROM Project_Assignments
GROUP BY Department
HAVING COUNT(*) > 3;

-- 5. Show departments whose maximum hours worked exceed 45.
SELECT Department, MAX(Hours_Worked) AS Max_Hours FROM Project_Assignments
GROUP BY Department
HAVING MAX(Hours_Worked) > 45;

-- 6. Find departments where the minimum hours worked is less than 30 and sort by minimum hours.
SELECT Department, MIN(Hours_Worked) AS Min_Hours FROM Project_Assignments
GROUP BY Department
...
SELECT Department,COUNT(*) AS Employee_Count,SUM(Hours_Worked) AS Total_Hours FROM Project_Assignments
GROUP BY Department
...
-- 9. Display departments having at least 3 employees.
-- Sort first by employee count (descending), then by department name (ascending).
SELECT Department, COUNT(*) AS Employee_Count FROM Project_Assignments
GROUP BY Department
HAVING COUNT(*) >= 3
...
SELECT Department,COUNT(*) AS Employee_Count,SUM(Hours_Worked) AS Total_Hours,AVG(Hours_Worked) AS Avg_Hours FROM Project_Assignments
GROUP BY Department
HAVING SUM(Hours_Worked) > 110
AND AVG(Hours_Worked) > 38
ORDER BY Avg_Hours DESC, Department ASC;